...
class TerminusView extends View {
  ...
  initialize(id, pwd, statusIcon, statusBar, shell, args = [], env = {}, autoRun = []) {
    ...
    this.subscriptions.add(atom.config.onDidChange('terminus.style.animationSpeed', this.setAnimationSpeed.bind(this)));

    this.updateToolbarVisibility();
    this.subscriptions.add(atom.config.onDidChange('terminus.toggles.showToolbar', this.updateToolbarVisibility.bind(this)));
    ...
    this.xterm.on('drop', this.recieveItemOrFile.bind(this));

    this.focusBindThis = this.focus.bind(this);
    this.on('focus', this.focusBindThis);
    this.subscriptions.add({dispose: () => {
      this.off('focus', this.focusBindThis);
    }});
  ...
  ptyPromise() {
    return new Promise((resolve, reject) => {
      if (this.ptyProcess) {
        this.ptyProcess.on("terminus:pty", resolve);
  ...
  attachWindowEvents() {
    this.onWindowResizeBindThis = this.onWindowResize.bind(this);
    $(window).on('resize', this.onWindowResizeBindThis);
  }

  detachWindowEvents() {
    $(window).off('resize', this.onWindowResizeBindThis);
  }

  attachResizeEvents() {
    this.resizeStartedBindThis = this.resizeStarted.bind(this);
    this.panelDivider.on('mousedown', this.resizeStartedBindThis);
  }

  detachResizeEvents() {
    this.panelDivider.off('mousedown', this.resizeStartedBindThis);
  }
  ...
  resizeStarted() {
    ...
    this.resizePanelBindThis = this.resizePanel.bind(this);
    this.resizeStoppedBindThis = this.resizeStopped.bind(this);
    $(document).on('mousemove', this.resizePanelBindThis);
    $(document).on('mouseup', this.resizeStoppedBindThis);
    this.xterm.css('transition', '');
  }

  resizeStopped() {
    $(document).off('mousemove', this.resizePanelBindThis);
    $(document).off('mouseup', this.resizeStoppedBindThis);
    this.xterm.css('transition', `height ${0.25 / this.animationSpeed}s linear`);
  }
